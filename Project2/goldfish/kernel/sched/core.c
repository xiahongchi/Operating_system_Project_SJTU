...
/*
 * part of the period that we allow ras tasks to run in us.
 * default: 0.2s
 */
int sysctl_sched_ras_runtime = 200000;
...
static void __sched_fork(struct task_struct *p)
{
	...
	if(p->sched_class == &ras_sched_class)
	printk(KERN_INFO "%d: using ras scheduling policy\n",p->pid);
...
__setscheduler(struct rq *rq, struct task_struct *p, int policy, int prio)
{
	...
	if(policy == SCHED_RAS){
		p->sched_class = &ras_sched_class;
		printk(KERN_INFO "change to ras scheduler: pid=%d\n",p->pid);
	}
...
/*
		...
		INIT_LIST_HEAD(&rq->leaf_ras_rq_list);
		init_tg_ras_entry(&root_task_group, &rq->rt, NULL, i, NULL);
#endif
*/
